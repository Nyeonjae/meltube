...

{
    ...
    if (typeof localStorage.getItem('rememberedEmail') === 'string') {
        $loginForm['email'].value = localStorage.getItem('rememberedEmail') // 이메일 저장기능
        $loginForm['rememberEmail'].checked = true; // 최초 로그인에 성공했을때 체크박스 활성화
        $loginForm['password'].focus(); // 이메일이 저장되어 있는 상태에서 비밀번호에 focus 부여
    }
    ...
    $loginForm.onsubmit = (e) => {
        ...
        $emailLabel.setValid($loginForm['email'].value.length >= 6 && $loginForm['email'].value.length <= 50);
        ...
        xhr.onreadystatechange = () => {
            ...
            if (response['result'] === 'success') {
                if ($loginForm['rememberEmail'].checked) {
                    ...
                    //  localStorage == 브라우저 자체에만 저장되는 데이터
                } else {
            ...
            const [title, content, onclick] = {
                ...
                failure_not_verified: ['로그인', `해당 계정의 이메일 인증이 완료되지 않았습니다. 이메일을 확인해주세요.<br><br>혹시 이메일이 오지 않았다면 인증 링크가 포함된 이메일을 <a href="/user/resend-register-email-token?email=${$loginForm['email'].value}" target="_blank">다시 전송</a>할 수 있습니다.`, ($dialog) => Dialog.hide($dialog)],
    ...
    $menu.querySelector(':scope > .item > [rel="register"]').onclick = (e) => {
        ...
        $cover.show();
        $registerForm.reset(); // 필드 초기화 // > 창을 내렸다가 다시 켰을때 초기화 해주는 역할
        $registerForm.show();
...
$registerForm.onsubmit = (e) => {
    ...
    xhr.onreadystatechange = () => {
        ...
        const [title, content, onclick] = {
            ...
            failure_duplicate_contact: ['회원가입', `입력하신 연락처 (${$registerForm['contact'].value})은 이미 사용중입니다. 다른 연락처을 사용해 주세요.`, ($dialog) => Dialog.hide($dialog)],
        ...
        }[response['result']] || ['오류', '서버가 알 수 없는 응답을 반환하였습니다. 잠시 후 다시 시도해주세요.',
...
$recoverForm.onsubmit = (e) => {
    ...
    if ($recoverForm['mode'].value === 'email') {
        ...
        $contactLabel.setValid($recoverForm['contact'].value.length >= 10 &&
            $recoverForm['contact'].value.length <= 12);
        ...
        xhr.onreadystatechange = () => {
            ...
            }[response['result']] || ['오류', '서버가 알 수 없는 응답을 반환하였습니다. 잠시 후 다시 시도해 주세요.',
                ($dialog) => Dialog.hide($dialog)];
            Dialog.show({
                title: title,
                content: content,
    ...
    if ($recoverForm['mode'].value === 'password') {
        ...
        xhr.onreadystatechange = () => {
            ...
            const [title, content, onclick] = {
                ...
                success: ['비밀번호 재설정', '입력하신 이메일로 비밀번호를 재설정 할 수 있는 링크를 포함한 메일을 전송하였습니다 <br><br>확인을 클릭하면 로그인 화면으로 돌아갑니다.', ($dialog) => {
                    Dialog.hide($dialog);
                    $cover.hide();
                    $recoverForm.hide();
                }],
