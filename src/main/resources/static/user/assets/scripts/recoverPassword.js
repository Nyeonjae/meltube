...
$recoverForm.onsubmit = (e) => {
    ...
    $passwordLabel.setValid($recoverForm['password'].value.length >= 6 &&
    $recoverForm['password'].value.length <= 50 , '올바른 비밀번호를 입력해 주세요.');
    ...
    xhr.onreadystatechange = () => {
        ...
        const[title, content, onclick] = {
            failure: ['비밀번호 재설정', `비밀번호를 재지정할 수 없습니다. 링크가 올바르지 않거나 링크가 손상되었을 수 있습니다.<br><br>동일한 문제가 반복된다면 비밀번호 재설정 링크를 다시 생성해 주세요.`,
                ($dialog) => Dialog.hide($dialog)],
            failure_expired: ['비밀번호 재설정', `비밀번호를 재설정할 수 없습니다. 링크는 더 이상 유효하지 않습니다. 멜튜브에서 인증 링크를 다시 받을 수 있습니다.`],
